...
SmallFW isn't very much a "Framework" at all, all it provides to you for classes is `Object` plus a small set of per-class runtime options. You can use these to write your own framework customised to your needs and desires!
...
SmallFW is EXTREMLEY configurable!
    ...
    --runtime-inline-value-storage=[y|n]    Use compact inline prefixes for embedded inline-value classes.
    ...
    --runtime-fast-objects=[y|n]            Enable fast-object allocation/release fast paths for classes that opt in via `+options`.
...
Classes opt into special runtime behavior by implementing `+ (SFClassOptions_t)options`.

Inline-value classes are stored inside their parent allocation instead of using a separate child allocation. This keeps related objects tightly grouped and improves cache locality.

    @interface MyValue : Object
...
Set `.fast_object = true` to opt a class into fast-object allocation/release paths, or `.trivial_release = true` for standalone trivial-release classes.
