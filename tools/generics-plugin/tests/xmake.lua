if not smallfw.is_wasm() then
    target("plugin-generic-metadata-checks")
        ...
        add_files("plugin-checks-stub.m")
        ...
        on_test(function (target)
            ...
            local init_source = projectdir .. "/tools/generics-plugin/tests/generic-ir-init-check.m"
            local exception_source = projectdir .. "/tools/generics-plugin/tests/generic-ir-exception-check.m"
            local module_cache = path.join(target:autogendir(), "plugin-ir-module-cache")
            local module_flags = {
                "-fmodules",
                "-fimplicit-modules",
            ...
            local function compile_ir(source, opt_level)
                ...
                append_all(args, module_flags)
                return os.iorunv(clang, args)
            end
