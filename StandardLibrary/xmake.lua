...
target("smallfw-stdlib")
    ...
    add_files(path.join(stdlib_src, "Collections", "Array.m"), {mflags = {"-fno-objc-arc"}})
    add_files(path.join(stdlib_src, "Collections", "List.m"), {mflags = {"-fno-objc-arc"}})
    add_files(path.join(stdlib_src, "Collections", "Map.m"), {mflags = {"-fno-objc-arc"}})
    add_files(path.join(stdlib_src, "Exceptions", "Exception.m"), {mflags = {"-fno-objc-arc"}})
    add_files(path.join(stdlib_src, "Number.m"), {mflags = {"-fno-objc-arc"}})
    add_files(path.join(stdlib_src, "Reflection", "Reflection.m"), {mflags = {"-fno-objc-arc"}})
