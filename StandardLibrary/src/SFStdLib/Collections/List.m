...
#import "SFStdLib/Exceptions/Exception.h"
...
- (instancetype)init
{
    return (id nonnil)[self initWithCapacity:0U];
}
...
- (SF_ERRORABLE(instancetype))initWithCapacity:(size_t)capacity
{
    ...
    auto tmp = (id *)self.allocator->alloc(self.allocator->ctx, bytes, alignof(id));
    if (tmp == nullptr) {
        [self release];
        SF_THROW([AllocationFailedException allocationFailedException]);
    }
...
- (SF_ERRORABLE(id))objectAtIndex:(size_t)idx
{
    if (idx >= _count or _items == nullptr) {
        SF_THROW([IndexOutOfBoundsException indexOutOfBoundsException]);
    }
    return _items[idx];
}
