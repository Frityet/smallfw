...
{
    auto ctx = (DispatchThreadCtx *)arg;
    long long sum = 0;
...
static bool case_dispatch_cache_warm_hits(void)
{
    auto calc_sel = sel_registerName("calc:");
...
static bool case_dispatch_super_lookup(void)
{
    auto ping_sel = sel_registerName("ping");
...
static bool case_dispatch_selector_equality(void)
{
    auto calc0 = sel_registerName("calc:");
    auto calc1 = sel_registerName("calc:");
    auto ping = sel_registerName("ping");
...
static bool case_dispatch_selector_lookup_only_registration(void)
{
    auto calc_sel = sel_registerName("calc:");
    auto ping_sel = sel_registerName("ping");
    auto missing = sel_registerName("definitely_missing_selector_name");
...
static bool case_dispatch_method_lookup_canonical(void)
{
    auto cls = (Class)objc_getClass("HotDispatch");
    auto canonical = sel_registerName("calc:");
...
static bool case_dispatch_concurrent_reads(void)
{
#if not SF_TEST_HAS_PTHREAD
        return 1;
#else
    enum { thread_count = 4,
...
static bool case_dispatch_c_msgsend_signatures(void)
{
#ifdef SF_DISPATCH_BACKEND_C
        auto zero_sel = sel_registerName("zero");
        auto take_i_sel = sel_registerName("takeI:");
        auto take_iq_sel = sel_registerName("takeIq:second:");
        auto take_q_sel = sel_registerName("takeQ:star:sel:");
        auto take_obj_sel = sel_registerName("takeObj:cls:ptr:cstr:");
        auto take_char_sel = sel_registerName("takeChar:");
        auto take_short_sel = sel_registerName("takeShort:");
        auto take_bool_sel = sel_registerName("takeBool:");
        auto take_c_sel = sel_registerName("takeC:");
        auto take_s_sel = sel_registerName("takeS:");
        auto take_long_sel = sel_registerName("takeLong:");
        auto take_ulong_sel = sel_registerName("takeULong:");
        auto take_pointer_sel = sel_registerName("takePointer:");
...
static bool case_dispatch_c_msgsend_double_arg(void)
{
#ifdef SF_DISPATCH_BACKEND_C
        auto take_double_sel = sel_registerName("takeDouble:");
        __unsafe_unretained CDispatchProbe *obj = SFW_NEW(CDispatchProbe);
        id result = nil;
        ...
        sf_test_reset_c_dispatch_probe();
        result = objc_msgSend(obj, take_double_sel, 9.0);
        objc_release(obj);
        return result == obj and sf_test_c_dispatch_probe_argc() == 1 and sf_test_c_dispatch_probe_value(0) == 9U;
#else
        return 1;
#endif
}

static bool case_dispatch_c_internal_helpers(void)
{
#ifdef SF_DISPATCH_BACKEND_C
        ...
        auto zero_sel = sel_registerName("zero");
        auto obj_class_ptr_sel = sel_registerName("takeObj:cls:ptr:cstr:");
        auto struct_sel = sel_registerName("takeStruct:");
        auto union_sel = sel_registerName("takeUnion:");
        auto double_sel = sel_registerName("takeDouble:");
        auto many_sel = sel_registerName("takeMany:second:third:fourth:fifth:");
        auto probe_class = (Class)objc_getClass("CDispatchProbe");

        if (zero_sel == nullptr or obj_class_ptr_sel == nullptr or struct_sel == nullptr or union_sel == nullptr or
            double_sel == nullptr or many_sel == nullptr or probe_class == Nil) {
            return 0;
        }
        ...
        unsupported = false;
        memset(codes, 0, sizeof(codes));
        if (sf_runtime_test_dispatch_collect_explicit_arg_codes(double_sel, codes, &unsupported) != 1 or unsupported != 0 or
            codes[0] != 'd') {
            return 0;
        }
...
static bool case_dispatch_struct_params(void)
{
...
static bool case_dispatch_struct_returns(void)
{
    __unsafe_unretained StructDispatchProbe *obj = SFW_NEW(StructDispatchProbe);
...
static bool case_dispatch_dtable_lookup(void)
{
    auto cls = (Class)objc_getClass("HotDispatch");
    auto calc_sel = sel_registerName("calc:");
    __unsafe_unretained HotDispatch *obj = SFW_NEW(HotDispatch);
...
static bool case_dispatch_forwarding_targets(void)
{
#if SF_RUNTIME_FORWARDING
        auto instance_sel = sel_registerName("forwardedValue:");
        auto class_sel = sel_registerName("classForwardedValue:");
        auto proxy_cls = (Class)objc_getClass("ForwardDispatchProxy");
...
static const SFTestCase g_dispatch_cases[] = {
    ...
    {"dispatch_c_msgsend_double_arg", case_dispatch_c_msgsend_double_arg},
