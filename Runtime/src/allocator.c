...
static inline size_t fast_bin_size(size_t bin)
{
...
static bool refill_fast_bin(size_t bin)
{
    ...
    const size_t slab_bytes = align_up(block_size * SF_FAST_ALLOC_REFILL_BLOCKS, page_size);
    const size_t block_count = slab_bytes / block_size;
    ...
    for (size_t i = block_count; i > 0U; --i) {
        auto node = (SFFreeNode_t *)(void *)(slab + ((i - 1U) * block_size));
