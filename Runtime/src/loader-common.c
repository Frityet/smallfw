...
typedef struct SFValueSlot {
    Class declared_cls;
    uint32_t owner_offset, storage_offset, storage_size, reserved;
} SFValueSlot_t;
...
static int sf_trace_class_layout_enabled(void)
{
    if (g_trace_class_layout < 0) {
        ...
        g_trace_class_layout = (value != nullptr and value[0] != '\0' and strcmp(value, "0") != 0) ? 1 : 0;
    }
...
static void sf_reset_class_caches_unlocked(void)
{
    memset((void *)g_layout_fixed_map, 0, sizeof(g_layout_fixed_map));
    memset((void *)g_layout_active_stack, 0, sizeof(g_layout_active_stack));
...
static void layout_active_pop(SFObjCClass_t *cls)
{
    ...
    for (size_t i = g_layout_active_count; i > 0; --i) {
        if (g_layout_active_stack[i - 1U] == cls) {
            memmove((void *)&g_layout_active_stack[i - 1U], (const void *)&g_layout_active_stack[i],
...
static size_t sf_fix_class_layout(SFObjCClass_t *cls)
{
        ...
        for (uintptr_t i = 0; i < list->count; ++i) {
            ...
            if (not disable_local_slots and ivar->offset != nullptr and adjusted_offset != INT32_MAX) {
                ...
                if (sf_extract_object_class_name(ivar->type, class_name, sizeof(class_name))) {
                    auto value_cls = (Class)sf_loader_class_lookup_unlocked(class_name);
    ...
    if (local_slot_count > 0) {
        ...
        if (super_slot_count > 0) {
            ...
            auto merged = (SFValueSlot_t *)sf_runtime_test_calloc(merged_count, sizeof(SFValueSlot_t));
            if (merged != nullptr) {
...
static void sf_build_class_dtable_unlocked(Class cls)
{
    ...
    if (c->superclass != nullptr and c->superclass != c and c->superclass->dtable != nullptr) {
        memcpy((void *)dtable, c->superclass->dtable, g_selector_count * sizeof(*dtable));
    }
                                         ...
                                         SFAllocator_t *allocator)
{
    ...
    for (size_t i = 0; i < slot_count; ++i) {
        ...
        auto owner_slot = (id *)(void *)(parent_bytes + slot->owner_offset);
        if (*owner_slot != nullptr) {
            continue;
        }

        auto hdr = (SFObjHeader_t *)(void *)(parent_bytes + slot->storage_offset);
        ...
        auto obj = sf_finish_object_alloc(cls, hdr);
                                     ...
                                     SFObjHeader_t *parent_hdr)
{
    ...
    if (parent_hdr->state != SF_OBJ_STATE_LIVE or sf_header_group_dead(root) or sf_header_group_live_count(root) == 0U) {
    ...
    int linked = sf_header_set_parent(hdr, parent);
    linked = linked and sf_header_set_group_root(hdr, root);
    linked = linked and sf_header_set_group_next(hdr, sf_header_group_head(root));
    linked = linked and sf_header_set_group_head(root, hdr); // NOLINT(readability-suspicious-call-argument)
    linked = linked and sf_header_set_group_live_count(root, sf_header_group_live_count(root) + 1U);
    if (not linked) {
        sf_runtime_mutex_unlock(group_lock);
...
static void sf_cache_class_meta(Class cls)
{
    ...
    slot->ivars = c->ivars;
    slot->encoding = sf_make_class_encoding(cls);
    sf_cache_method_encoding_list(c->methods);
    sf_cache_ivar_encoding_list((SFObjCIvarList_t *)c->ivars);
    ...
    if (total_count == 0U and slot->cxx_destruct_imp == nullptr and(slot->dealloc_imp == nullptr or slot->dealloc_imp == base_dealloc_imp)) {
...
static struct SFEncoding sf_make_class_encoding(Class cls)
{
    static const char object_encoding_source[] = "@";
    if (cls == nullptr) {
        return (struct SFEncoding){.type = {.kind = SF_ENCODING_KIND_INVALID}};
    }
    return (struct SFEncoding){.type = {.valid = true, .kind = SF_ENCODING_KIND_OBJECT, .code = '@'},
                               .source = {.bytes = object_encoding_source, .length = 1U},
...
static void sf_cache_method_encoding_list(SFObjCMethodList_t *list)
{
    while (list != nullptr) {
        for (int32_t i = 0; i < list->count; ++i) {
            if (list->methods[i].types != nullptr) {
                (void)sf_method_encoding_parse(list->methods[i].types);
            }
        }
        list = list->next;
    }
...
static void sf_cache_ivar_encoding_list(SFObjCIvarList_t *list)
{
    ...
    size_t stride = list->item_size > 0 ? (size_t)list->item_size : sizeof(SFObjCIvar_t);
    auto cursor = (unsigned char *)list->ivars;
    for (uintptr_t i = 0; i < list->count; ++i) {
        auto ivar = (SFObjCIvar_t *)(void *)cursor;
        if (ivar->type != nullptr) {
            (void)sf_encoding_parse(ivar->type);
        }
        cursor += stride;
    }
}
...
Class *objc_copyClassList(unsigned int *outCount)
{
    ...
    size_t cap = 16;
    size_t count = 0;
    auto list = (Class *)sf_runtime_test_malloc(cap * sizeof(Class));
    if (list == nullptr) {
        return nullptr;
    }
    ...
    for (size_t i = 0; i < SF_CLASS_MAP_CAPACITY; ++i) {
        ...
        if (count == cap) {
            ...
            auto next = (Class *)sf_runtime_test_realloc((void *)list, next_cap * sizeof(Class));
            if (next == nullptr) {
                free((void *)list);
                return nullptr;
            }
    ...
    auto exact = (Class *)sf_runtime_test_realloc((void *)list, count * sizeof(Class));
...
Method *class_copyMethodList(Class cls, unsigned int *outCount)
{
    ...
    auto arr = (Method *)sf_runtime_test_malloc(count * sizeof(Method));
    if (arr == nullptr) {
        return nullptr;
    }
...
Ivar class_getInstanceVariable(Class cls, const char *name)
{
    auto c = (SFObjCClass_t *)cls;
    while (c != nullptr) {
        auto list = (SFObjCIvarList_t *)c->ivars;
        if (list != nullptr and list->count > 0) {
            size_t stride = (size_t)list->item_size;
            if (stride < sizeof(SFObjCIvar_t))
                stride = sizeof(SFObjCIvar_t);
...
Ivar *class_copyIvarList(Class cls, unsigned int *outCount)
{
    ...
    auto arr = (Ivar *)sf_runtime_test_malloc(count * sizeof(Ivar));
    if (arr == nullptr) {
        return nullptr;
    }
