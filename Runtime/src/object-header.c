...
#    if SF_RUNTIME_INLINE_VALUE_STORAGE
            ...
            return hdr != nullptr and(inline_hdr->flags & SF_OBJ_FLAG_INLINE_VALUE) != 0U and(inline_hdr->tagged_parent & (uintptr_t)1U) != 0U;
#    else
            (void)hdr;
            return 0;
#    endif
...
#        endif
        ...
        static bool sf_header_set_generic_type_class_local(SFObjHeader_t *hdr, Class cls)
        {
            SFObjColdState_t *cold = nullptr;
...
#    endif
    ...
    static bool sf_header_set_allocator_local(SFObjHeader_t *hdr, SFAllocator_t *allocator)
    {
        SFObjColdState_t *cold = nullptr;
    ...
    static bool sf_header_set_parent_local(SFObjHeader_t *hdr, id parent)
    {
        SFObjColdState_t *cold = nullptr;
    ...
    static bool sf_header_set_group_next_local(SFObjHeader_t *hdr, SFObjHeader_t *group_next)
    {
        SFObjColdState_t *cold = nullptr;
...
#    if SF_RUNTIME_THREADSAFE or not SF_RUNTIME_INLINE_GROUP_STATE
        ...
        static bool sf_header_set_group_state_local(SFObjHeader_t *root, SFGroupState_t *group)
        {
            SFObjColdState_t *cold = nullptr;
...
#    if SF_RUNTIME_THREADSAFE
        static SFGroupState_t *nillable sf_header_group_state_atomic(SFObjHeader_t *nillable hdr)
        {
            return hdr != nullptr ? atomic_load_explicit((_Atomic(SFGroupState_t *) *)&hdr->group, memory_order_acquire) : nullptr;
        }

        static void sf_header_set_group_state_atomic(SFObjHeader_t *nillable hdr, SFGroupState_t *nillable group)
        {
            if (hdr != nullptr) {
                atomic_store_explicit((_Atomic(SFGroupState_t *) *)&hdr->group, group, memory_order_release);
            }
        }

        static SFGroupState_t *nillable sf_header_exchange_group_state_atomic(SFObjHeader_t *nillable hdr, SFGroupState_t *nillable group)
        {
            return hdr != nullptr ? atomic_exchange_explicit((_Atomic(SFGroupState_t *) *)&hdr->group, group, memory_order_acq_rel) : nullptr;
        }

        static bool sf_header_try_set_group_state_atomic(SFObjHeader_t *nillable hdr, SFGroupState_t *nillable group)
        {
            ...
            return atomic_compare_exchange_strong_explicit((_Atomic(SFGroupState_t *) *)&hdr->group, &expected, group, memory_order_release, memory_order_acquire);
        }
#    else
        static bool sf_header_uses_inline_group_state(SFObjHeader_t *nillable hdr)
        {
            return hdr != nullptr and(((uintptr_t)hdr->group) & (uintptr_t)1U) != 0U;
        }

        static SFObjHeader_t *nillable sf_header_inline_group_root(SFObjHeader_t *nillable hdr)
        {
        ...
        static bool sf_header_can_use_inline_group_root(SFObjHeader_t *nillable hdr)
        {
            return hdr != nullptr and(hdr->flags & SF_OBJ_FLAG_EMBEDDED) == 0U;
        }

        static SFObjHeader_t *nillable sf_header_inline_group_head(SFObjHeader_t *nillable root)
        {
            return root != nullptr ? (SFObjHeader_t *)(void *)root->parent : nullptr;
        }

        static void sf_header_set_inline_group_head(SFObjHeader_t *nillable root, SFObjHeader_t *nillable head)
        {
            if (root != nullptr) {
...
static SFGroupState_t *nillable sf_create_group_state(SFObjHeader_t *nonnil root)
{
    auto group = (SFGroupState_t *)sf_runtime_test_calloc(1, sizeof(SFGroupState_t));
    if (group == nullptr) {
        return nullptr;
    }
    group->root = root;
...
#    if SF_RUNTIME_VALIDATION
            auto cold = (next != nullptr) ? sf_header_ensure_cold_state(hdr) : sf_header_cold_state(hdr);
            if (cold != nullptr) {
                cold->live_next = next;
            }
#    else
...
#else
        if (sf_header_uses_inline_group_state(hdr) and sf_header_inline_group_root(hdr) == hdr and(hdr->flags & SF_OBJ_FLAG_EMBEDDED) == 0U) {
            return nullptr;
        }
        return hdr->parent;
#endif
...
#    else
            ...
            return hdr == sf_header_group_root_local(hdr) and cold != nullptr and(cold->inline_group_live_count != 0U or cold->inline_group_head != nullptr);
#    endif
#else
...
#    if SF_RUNTIME_THREADSAFE or not SF_RUNTIME_INLINE_GROUP_STATE
            ...
            SFGroupState_t *group = sf_create_group_state((SFObjHeader_t *nonnil)hdr);
            if (group == nullptr) {
                return 0;
            }
...
#    endif
        ...
        group = sf_create_group_state((SFObjHeader_t *nonnil)hdr);
        if (group == nullptr) {
            return 0;
        }
        hdr->group_next = nullptr;
