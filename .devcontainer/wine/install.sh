...
add_wine_repo() {
  suite="$1"  # e.g. 'debian' or 'ubuntu' or specific distro identifier used by WineHQ
  keyring="/usr/share/keyrings/winehq-archive-keyring.gpg"

  # fetch and store key in keyring (GPG format)
  curl -fsSL https://dl.winehq.org/wine-builds/winehq.key | gpg --dearmor -o "$keyring"

  # write repo using signed-by
  echo "deb [signed-by=$keyring] https://dl.winehq.org/wine-builds/$suite $VERSION_CODENAME main" \
